...
public class CommandLineAppStartupRunner implements CommandLineRunner {
    ...
    public CommandLineAppStartupRunner(CooperativeService coopService, ProductService prodService, CooperativeProductService coopProdService, AnnualResultService annualResultService, CategoryService categoryService, UserRepository userRepository) {
    ...
    public void run(String... args) throws Exception {
        ...
        if (!dbAlreadyStarted) {
            ...
            annualResultService.save(AnnualResultEntity.builder().annualResultPK(AnnualResultPK.builder().idCoop(coop1).year(2020).build()).result(new BigDecimal(800)).build());
            ...
            UserEntity userEntity2 = UserEntity.builder().document("admin-sicredi").password(new MD5Crypt().encode("Senha@123")).role("admin").codCoop(coop1).name("Admin Sicredi").build();
